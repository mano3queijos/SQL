...
SELECT cidade, CASE WHEN cidade = 'Salvador' THEN 'Nordeste' WHEN cidade = 'Rio de Janeiro' THEN 'Sudeste' END AS regiao FROM clientes

UPDATE clientes SET cidade = 'Rio de Janeiro' where clientes.id = 2

SELECT valor_total, CASE WHEN valor_total >= 200 THEN 'Caro' WHEN valor_total <200 THEN 'Barato' END AS faixa_de_preco FROM pedidos

INSERT INTO clientes (nome, idade, cidade) VALUES ('Carlinhos', 45, 'Acre');

COPY clientes TO 'C:\Users\Public\marcao_e_meu_mano_4queijos.csv' WITH DELIMITER ';' CSV HEADER;